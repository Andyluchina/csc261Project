...
function getSQLString($privalege,$wrkID){
  ...
  if($privalege==2){
    ...
    $sql="SELECT FNAME,MI,LNAME,PHONE_NUM,TITLE FROM EMPLOYEE WHERE WORK_ID in (SELECT EMPLOYEE_ID FROM WORKS_ON WHERE PROJ_ID in (SELECT PROJ_ID FROM PROJECT WHERE MISSION_ID IN(SELECT MISSION_ID FROM MISSION WHERE MLEADER_ID=".$wrkID."))) AND (SELECT * FROM EMPLOYEE WHERE WORK_ID=".$wrkID.");";
    return $sql;
