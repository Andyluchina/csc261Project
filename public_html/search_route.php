...
function traverseArray($privalege,$string,$attributes,$boolean,$tablename){
	...
	if($check==1){
		if($privalege==3||$privalege==4||$privalege==5||$privalege==6||$privalege==7||$privalege==8||$privalege==9||$privalege==10||$privalege==11){
...
function getSQLString($privalege,$srchSTR,$wrkID, $boolean){
	...
	if($privalege==2){
		...
		if($boolean){
			$sql="SELECT * FROM EMPLOYEE ".$srchSTR." WORK_ID=$wrkID UNION SELECT * FROM EMPLOYEE ".$srchSTR." WORK_ID IN ((SELECT EMPLOYEE_ID FROM WORKS_ON WHERE PROJ_ID IN (SELECT PROJ_ID FROM PROJECT WHERE PLEADER_ID=".$wrkID.")) AND NOT IN (SELECT EMPLOYEE_ID FROM WORKS_ON) AND TITLE!='Mission Leader' AND TITLE!='Administrator');";
		}
		else{
    	...
    	$sql="SELECT * FROM SUPPLIES ".$srchSTR." PROJ_ID in(SELECT PROJ_ID FROM PROJECT WHERE PLEADER_ID=".$wrkID.");";
    return $sql;
 	}
 	else if($privalege==11){
    	$sql="SELECT * FROM SUPPLIES ".$srchSTR." PROJ_ID in(SELECT PROJ_ID FROM PROJECT WHERE MISSION_ID IN(SELECT MISSION_ID FROM MISSION WHERE MLEADER_ID=".$wrkID."));";
    return $sql;
 	}
