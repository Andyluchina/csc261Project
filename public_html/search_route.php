...
function getSQLString($privalege,$srchSTR,$wrkID){
	...
	if($privalege==2){
		...
		$sql="SELECT * FROM EMPLOYEE ".$srchSTR." UNION SELECT * FROM EMPLOYEE ".$srchSTR." WORK_ID in (SELECT EMPLOYEE_ID FROM WORKS_ON WHERE PROJ_ID in (SELECT PROJ_ID FROM PROJECT WHERE MISSION_ID IN(SELECT MISSION_ID FROM MISSION WHERE MLEADER_ID=".$wrkID.")));";
		return $sql;

	}
	else if($privalege==4){
		$sql="SELECT * FROM EMPLOYEE".$srchSTR." UNION SELECT * FROM EMPLOYEE ".$srchSTR." WORK_ID IN (SELECT EMPLOYEE_ID FROM WORKS_ON WHERE PROJ_ID IN (SELECT PROJ_ID FROM PROJECT WHERE PLEADER_ID=".$wrkID."));";
		return $sql;
